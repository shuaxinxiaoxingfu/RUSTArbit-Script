...
bool ImGui::KeyBind(const char* label, int& k, const ImVec2& size_arg)
{
    ...
    if (k != 0 && g.ActiveId != id) {
        ...
        strcpy_s(buf_display, "Bir Tusa Bas");
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(5, 4), check_bb.Max + ImVec2(-3, -4), ImColor(255, 255, 255));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 20), window->DC.CursorPos + ImVec2(w, 28));
    ...
    // Draw frame
   // draw->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 4), ImColor(45, 45, 45), 3);
    window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 0.5f), ImVec2(frame_bb.Max.x - 3, frame_bb.Max.y), ImColor(70, 70, 70), ImColor(70, 70, 70), ImColor(50, 50, 50), ImColor(50, 50, 50));
    window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 0.5f), ImVec2(grab_bb.Min.x + 2, frame_bb.Min.y + 8), ImColor(255, 255, 255), ImColor(255, 255, 255), ImColor(255, 255, 255), ImColor(255, 255, 255));


    // draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(45, 45, 45), 5, 0, 0);
    ...
    // RenderTextClipped(ImVec2(frame_bb.Max.x  , frame_bb.Min.y ), ImVec2(frame_bb.Max.x , frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    RenderTextClipped(ImVec2(grab_bb.Min.x - 15, total_bb.Min.y + 8), ImVec2(grab_bb.Max.x + 14, total_bb.Min.y + 20), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

    // Slider grab
    window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, frame_bb.Min.y - 2), ImVec2(grab_bb.Max.x + 2, frame_bb.Max.y + 2), ImColor(54, 100, 139), 10);



    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, total_bb.Min.y - 20), label);
